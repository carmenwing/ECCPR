...
int main()
{
    ...
    z = n;
        ...
        for( i = 0; i < n; i++)
            {
                scanf ("%s %d %d %d %d\n", concert[i].oras, &concert[i].zi, &concert[i].luna, &concert[i].pret_bilet, &concert[i].distanta );
            }
     while(z--)
    {    
        ...
        if (min_initializat == 0)
            {
                for( i = 0; i < n; i++)
                    {
                        if ( (concert[i].luna < luna_in) || (concert[i].luna > luna_sf) || ((concert[i].luna == luna_in) && ((concert[i].zi < zi_in) || (concert[i].zi > zi_sf))))
                            {
