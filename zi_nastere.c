...
int main() {
  ...
  for (i = 0; i < nr - 1; i++) {
    for (j = i + 1; j < nr; j++) {
      if ((aniv[i].luna > aniv[j].luna) ||
          ((aniv[i].luna == aniv[j].luna) && (aniv[i].zi > aniv[j].zi))) {
        min = aniv[i];
        aniv[i] = aniv[j];
        aniv[j] = min;
      }
  ...
  // SAU  printf("%02d-%02d\n", aniv[i].zi, aniv[i].luna );   - afiseaza nr de
  // max 2 cifre si completeaza cu cati de 0 este nevoie
}
